    ...
    async handlePayment() {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          const checkoutWindow = window.open(paymentUrl, '_parent', 'width=600,height=400,resizable=yes,scrollbars=yes');
          ...
          this.paymentError = response.data.error || 'An error occurred. Please try again later.';
        }
      } catch (error) {
