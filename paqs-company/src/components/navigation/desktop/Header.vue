<template>
  <q-header bordered :class="classes({ prefix: 'bg', invert: true })">
    <q-toolbar>
      <Button
        v-if="$q.screen.lt.md"
        icon="menu"
      ...
      <template v-else>
        ...
        <Button icon="logout" aria-label="Logout" flat @click="logout" />
      </template>
    </q-toolbar>
  </q-header>
</template>
...
export default defineComponent({
  ...
  components: {
    ...
    Links: defineAsyncComponent(() =>
      import("components/navigation/desktop/Link.vue")
    ),
